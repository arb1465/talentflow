    ...
    <Container maxWidth="lg" sx={{ py: 3 }}>
      ...
      <Paper sx={{ p: 4, borderRadius: 2 }}>
        <Grid container spacing={4}>
          {/* Left Column - now uses dynamic 'job' data */}
          <Grid item xs={12} md={8}>
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
              <Avatar sx={{ width: 64, height: 64, mr: 2, bgcolor: 'primary.main' }} src={job.company.avatarUrl}>
                {job.company.name.charAt(0)}
              </Avatar>
              <Box>
                <Typography variant="h3" component="h1" sx={{ fontWeight: 'bold' }}>{job.title}</Typography>
                <Typography variant="h5" color="text.secondary">{job.company.name}</Typography>
              </Box>
            </Box>
            <Divider sx={{ my: 3 }} />
